...
- **Deploy after any change to snippet source files.** The blog's popover code snippets are extracted from Python and R source files listed in `extract_snippets.py` (`tests/test_identities.py`, `bench_timing.py`, `bench_samplers.py`, `conditional_poisson/torch.py`, `conditional_poisson/numpy.py`, `bench_timing_r.R`). Any change to these files changes the deployed snippets and requires `blog deploy`.
