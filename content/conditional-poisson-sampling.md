...
**Precomputation.** Recall that the tree already stores the coefficients of each node's polynomial, and the parent's $k\textsuperscript{th}$ coefficient is the convolution $\llbracket P \rrbracket(\z^k) = \sum_{j=0}^{k} \llbracket P_L \rrbracket(\z^j) \cdot \llbracket P_R \rrbracket(\z^{k-j})$.  The individual terms of this sum are the (unnormalized) PMF of the split distribution—we just need to normalize and cumsum:
...
$\quad$ $p_j \leftarrow b\text{.poly}[j] \cdot c\text{.poly}[k{-}j]$ <b>for</b> $j \in 0, \ldots, k$<br>
