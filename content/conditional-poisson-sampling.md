...
**Algebraic proof for small $N$.**  For fixed $N$, the bound reduces to a polynomial nonnegativity condition: $(\text{denom} + \text{numer})(\text{denom} - \text{numer}) \geq 0$ on the feasible polytope $\{p_j \in (0,1) : \sum p_j = n\}$.  Both factors are non-negative for all $N \leq 6$ and all $n < N$ (verified by random sampling with $>500\text{k}$ points per $(N,n)$ pair, zero violations).  For $N = 3$, the nonnegativity is proved symbolically via polynomial factoring.  For general $N$, the polynomial has $N-2$ variables and grows exponentially, so a structural argument (e.g., induction via deletion-contraction) is needed.

**Tilting decomposition.**  By exponential tilting to the midpoint mean $n - 1/2$: $\log R_i = -t_{\text{mid}} + \log(P_t(n)/P_t(n{-}1))$.  The Gaussian contribution is $|t_{\text{mid}}| \approx |p_i - 1/2|/\tilde{d}$, and the tilted ratio $P_t(n)/P_t(n{-}1) - 1$ is controlled by the third cumulant, contributing $\leq 1/(2\tilde{d})$ from the first Edgeworth correction.  The "slack" $\max(p_i, 1{-}p_i) - |p_i - 1/2| = 1/2$ in the numerically verified bound $|\log R_i| \leq \max(p_i, 1{-}p_i)/\tilde{d}$ exactly absorbs this correction.  Making this fully rigorous requires explicit higher-order Edgeworth bounds.<a href="test_identities.py#test_poisson_approximation_bound" title="test_poisson_approximation_bound" class="verified" target="_blank">✓</a>
