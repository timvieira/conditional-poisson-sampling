...
**Polynomials as arrays.** A polynomial is just a fancy array: `[1, 3, 2]` represents $1 + 3\z + 2\z^2$.  The $\z$ is bookkeeping—it gives the array a notion of *multiplication* (convolution) that predates computers by centuries.  Euler, Cauchy, and their contemporaries used generating functions to manipulate sequences long before anyone had a word for "data structure."  The product tree is this old idea applied at scale: multiplying two polynomials is convolution of their coefficient arrays—for example, $(1 + 2\z)(1 + 3\z)$ corresponds to convolving `[1, 2]` with `[1, 3]` to get `[1, 5, 6]`.  Using the fast Fourier transform (FFT), this convolution costs $\mathcal{O}(d \log d)$ where $d$ is the degree, rather than $\mathcal{O}(d^2)$ for the schoolbook method.  This is the key fact that makes the product tree $\mathcal{O}(N \log^2 n)$ instead of $\mathcal{O}(Nn)$.
