...
**Software.** The code is available as a [Python library](https://github.com/timvieira/conditional-poisson-sampling).  As far as I can tell, this is the only publicly available library for conditional Poisson sampling in Python (or any language outside of R's survey-sampling packages).  The existing R implementation—`UPmaxentropy` in the [sampling](https://cran.r-project.org/web/packages/sampling/) package—uses $\mathcal{O}(Nn)$ dynamic programming.  The product-tree algorithm used here does not appear in any prior software that I could find.
...
.entry-content h2, .entry-content h3, .entry-content h4 { text-align: left; }
...
<div class="pseudocode">
<b>def</b> $\text{sample}(a, k)$: <span style="color:#888; font-size:0.85em">// sample $k$ items from subtree $a$</span><br>
$\quad$ <b>if</b> $a$ is a leaf: <b>return</b> $\{a\text{.item}\}$ if $k = 1$, else $\emptyset$<br>
$\quad$ Let $b, c$ be the children of $a$<br>
$\quad$ $p_j \leftarrow \llbracket P_b \rrbracket(\z^j) \cdot \llbracket P_c \rrbracket(\z^{k-j})$ <b>for</b> $j = 0, \ldots, k$<br>
$\quad$ Draw $j \sim \text{Categorical}(\boldsymbol{p})$<br>
$\quad$ <b>return</b> $\text{sample}(b, j) \cup \text{sample}(c, k{-}j)$<br>
<br>
$S \leftarrow \text{sample}(\text{root}, n)$
</div>

This is exact, not approximate—it follows from the **weighted Vandermonde identity**: $\Zw{(\ba;\, \bb)}{k} = \sum_{j=0}^{k} \Zw{\ba}{j} \cdot \Zw{\bb}{k-j}$, which says the number of ways to choose $k$ items from $A \cup B$ is the convolution of the two groups' counts.  Each term $p_j$ is proportional to the conditional probability of assigning $j$ items to the left child and $k - j$ to the right.  At the leaves, quota 1 means "include this item"; quota 0 means "exclude."

The tree is built once ($\mathcal{O}(N \log^2 N)$) and reused across samples.  Each sample costs $\mathcal{O}(n \log N)$: at each of the $\mathcal{O}(\log N)$ levels, only nodes with nonzero quota are visited, and the total quota per level is $n$.
...
For fair comparison, the plot shows both "including setup" and "excluding setup" variants.  R's [`UPMEqfromw`](https://github.com/cran/sampling/blob/master/R/UPMEqfromw.R) computes the $\mathcal{O}(Nn)$ DP table; [`UPMEsfromq`](https://github.com/cran/sampling/blob/master/R/UPMEsfromq.R) draws one $\mathcal{O}(N)$ sequential sample from it.  Our product tree costs $\mathcal{O}(N \log^2 n)$ to build plus $\mathcal{O}(Nn)$ to precompute the CDF tables, then $\mathcal{O}(n \log N)$ per sample via quota splitting with binary search on the cached CDFs.  The "incl. build/DP" lines give apples-to-apples single-sample cost; the "excl." lines show the marginal cost of additional samples from a precomputed structure.
...
<script>
(function() {
var raw = [["DP forward", "Z", 50, 5, 0.2137], ["NumPy tree", "Z", 50, 5, 3.5566], ["PyTorch FFT", "Z", 50, 5, 1.8522], ["DP forward", "pi", 50, 5, 0.6744], ["NumPy tree", "pi", 50, 5, 21.4044], ["PyTorch FFT + autograd", "pi", 50, 5, 8.9727], ["NumPy tree (L-BFGS)", "fit", 50, 5, 1115.4894], ["R:sampling (pi)", "pi", 50, 5, 0.4692], ["R:sampling (fit)", "fit", 50, 5, 2.0], ["R:sampling (1 sample, incl. DP)", "samples", 50, 5, 0.3532], ["R:sampling (1 sample, excl. DP)", "samples", 50, 5, 0.0752], ["NumPy tree (1 sample)", "samples", 50, 5, 0.0245], ["PyTorch tree (1 sample)", "samples", 50, 5, 0.0349], ["Sequential (1 sample)", "samples", 50, 5, 0.036], ["DP forward", "Z", 50, 10, 0.2029], ["NumPy tree", "Z", 50, 10, 3.0838], ["PyTorch FFT", "Z", 50, 10, 2.407], ["DP forward", "pi", 50, 10, 1.6584], ["NumPy tree", "pi", 50, 10, 11.8119], ["PyTorch FFT + autograd", "pi", 50, 10, 6.8692], ["NumPy tree (L-BFGS)", "fit", 50, 10, 678.3815], ["R:sampling (pi)", "pi", 50, 10, 1.0], ["R:sampling (fit)", "fit", 50, 10, 5.0], ["R:sampling (1 sample, incl. DP)", "samples", 50, 10, 0.5], ["R:sampling (1 sample, excl. DP)", "samples", 50, 10, 0.091], ["NumPy tree (1 sample)", "samples", 50, 10, 0.0882], ["PyTorch tree (1 sample)", "samples", 50, 10, 0.0872], ["Sequential (1 sample)", "samples", 50, 10, 0.0451], ["DP forward", "Z", 50, 20, 0.2098], ["NumPy tree", "Z", 50, 20, 3.7427], ["PyTorch FFT", "Z", 50, 20, 2.3288], ["DP forward", "pi", 50, 20, 2.6822], ["NumPy tree", "pi", 50, 20, 8.7769], ["PyTorch FFT + autograd", "pi", 50, 20, 4.889], ["NumPy tree (L-BFGS)", "fit", 50, 20, 547.1085], ["R:sampling (pi)", "pi", 50, 20, 1.0], ["R:sampling (fit)", "fit", 50, 20, 8.0], ["R:sampling (1 sample, incl. DP)", "samples", 50, 20, 1.0], ["R:sampling (1 sample, excl. DP)", "samples", 50, 20, 0.081], ["NumPy tree (1 sample)", "samples", 50, 20, 0.1161], ["PyTorch tree (1 sample)", "samples", 50, 20, 0.1427], ["Sequential (1 sample)", "samples", 50, 20, 0.046], ["DP forward", "Z", 100, 5, 0.3283], ["NumPy tree", "Z", 100, 5, 6.7727], ["PyTorch FFT", "Z", 100, 5, 4.3903], ["DP forward", "pi", 100, 5, 1.163], ["NumPy tree", "pi", 100, 5, 18.409], ["PyTorch FFT + autograd", "pi", 100, 5, 8.5281], ["NumPy tree (L-BFGS)", "fit", 100, 5, 2501.9779], ["R:sampling (pi)", "pi", 100, 5, 1.0], ["R:sampling (fit)", "fit", 100, 5, 6.0], ["R:sampling (1 sample, incl. DP)", "samples", 100, 5, 1.0], ["R:sampling (1 sample, excl. DP)", "samples", 100, 5, 0.1348], ["NumPy tree (1 sample)", "samples", 100, 5, 0.075], ["PyTorch tree (1 sample)", "samples", 100, 5, 0.0474], ["Sequential (1 sample)", "samples", 100, 5, 0.0593], ["DP forward", "Z", 100, 10, 0.3629], ["NumPy tree", "Z", 100, 10, 4.2306], ["PyTorch FFT", "Z", 100, 10, 5.2191], ["DP forward", "pi", 100, 10, 3.6593], ["NumPy tree", "pi", 100, 10, 25.1636], ["PyTorch FFT + autograd", "pi", 100, 10, 8.4247], ["NumPy tree (L-BFGS)", "fit", 100, 10, 3198.3147], ["R:sampling (pi)", "pi", 100, 10, 1.0], ["R:sampling (fit)", "fit", 100, 10, 9.0], ["R:sampling (1 sample, incl. DP)", "samples", 100, 10, 1.0], ["R:sampling (1 sample, excl. DP)", "samples", 100, 10, 0.171], ["NumPy tree (1 sample)", "samples", 100, 10, 0.0511], ["PyTorch tree (1 sample)", "samples", 100, 10, 0.0642], ["Sequential (1 sample)", "samples", 100, 10, 0.0569], ["DP forward", "Z", 100, 20, 0.3077], ["NumPy tree", "Z", 100, 20, 5.8869], ["PyTorch FFT", "Z", 100, 20, 2.36], ["DP forward", "pi", 100, 20, 3.1028], ["NumPy tree", "pi", 100, 20, 17.9566], ["PyTorch FFT + autograd", "pi", 100, 20, 5.3383], ["NumPy tree (L-BFGS)", "fit", 100, 20, 1990.2996], ["R:sampling (pi)", "pi", 100, 20, 2.0], ["R:sampling (fit)", "fit", 100, 20, 6.0], ["R:sampling (1 sample, incl. DP)", "samples", 100, 20, 1.0], ["R:sampling (1 sample, excl. DP)", "samples", 100, 20, 0.0954], ["NumPy tree (1 sample)", "samples", 100, 20, 0.0919], ["PyTorch tree (1 sample)", "samples", 100, 20, 0.1086], ["Sequential (1 sample)", "samples", 100, 20, 0.0645], ["DP forward", "Z", 100, 50, 0.3549], ["NumPy tree", "Z", 100, 50, 4.2802], ["PyTorch FFT", "Z", 100, 50, 2.2628], ["DP forward", "pi", 100, 50, 7.8227], ["NumPy tree", "pi", 100, 50, 11.827], ["PyTorch FFT + autograd", "pi", 100, 50, 7.7421], ["NumPy tree (L-BFGS)", "fit", 100, 50, 1464.9001], ["R:sampling (pi)", "pi", 100, 50, 7.0], ["R:sampling (fit)", "fit", 100, 50, 28.0], ["R:sampling (1 sample, incl. DP)", "samples", 100, 50, 3.0], ["R:sampling (1 sample, excl. DP)", "samples", 100, 50, 0.178], ["NumPy tree (1 sample)", "samples", 100, 50, 0.3434], ["PyTorch tree (1 sample)", "samples", 100, 50, 0.3121], ["Sequential (1 sample)", "samples", 100, 50, 0.1045], ["DP forward", "Z", 200, 5, 0.8214], ["NumPy tree", "Z", 200, 5, 13.9527], ["PyTorch FFT", "Z", 200, 5, 7.3852], ["DP forward", "pi", 200, 5, 3.9692], ["NumPy tree", "pi", 200, 5, 38.0803], ["PyTorch FFT + autograd", "pi", 200, 5, 29.3787], ["NumPy tree (L-BFGS)", "fit", 200, 5, 6887.1283], ["R:sampling (pi)", "pi", 200, 5, 1.0], ["R:sampling (fit)", "fit", 200, 5, 6.0], ["R:sampling (1 sample, incl. DP)", "samples", 200, 5, 1.0], ["R:sampling (1 sample, excl. DP)", "samples", 200, 5, 0.2364], ["NumPy tree (1 sample)", "samples", 200, 5, 0.0869], ["PyTorch tree (1 sample)", "samples", 200, 5, 0.0422], ["Sequential (1 sample)", "samples", 200, 5, 0.0742], ["DP forward", "Z", 200, 10, 0.7748], ["NumPy tree", "Z", 200, 10, 14.0284], ["PyTorch FFT", "Z", 200, 10, 11.9972], ["DP forward", "pi", 200, 10, 12.7294], ["NumPy tree", "pi", 200, 10, 23.4555], ["PyTorch FFT + autograd", "pi", 200, 10, 8.364], ["NumPy tree (L-BFGS)", "fit", 200, 10, 5733.5787], ["R:sampling (pi)", "pi", 200, 10, 2.0], ["R:sampling (fit)", "fit", 200, 10, 8.0], ["R:sampling (1 sample, incl. DP)", "samples", 200, 10, 1.0], ["R:sampling (1 sample, excl. DP)", "samples", 200, 10, 0.1936], ["NumPy tree (1 sample)", "samples", 200, 10, 0.0693], ["PyTorch tree (1 sample)", "samples", 200, 10, 0.0838], ["Sequential (1 sample)", "samples", 200, 10, 0.111], ["DP forward", "Z", 200, 20, 0.461], ["NumPy tree", "Z", 200, 20, 7.8675], ["PyTorch FFT", "Z", 200, 20, 4.0048], ["DP forward", "pi", 200, 20, 5.9563], ["NumPy tree", "pi", 200, 20, 22.1174], ["PyTorch FFT + autograd", "pi", 200, 20, 10.8998], ["NumPy tree (L-BFGS)", "fit", 200, 20, 4490.867], ["R:sampling (pi)", "pi", 200, 20, 4.0], ["R:sampling (fit)", "fit", 200, 20, 24.0], ["R:sampling (1 sample, incl. DP)", "samples", 200, 20, 4.0], ["R:sampling (1 sample, excl. DP)", "samples", 200, 20, 0.2808], ["NumPy tree (1 sample)", "samples", 200, 20, 0.1002], ["PyTorch tree (1 sample)", "samples", 200, 20, 0.1114], ["Sequential (1 sample)", "samples", 200, 20, 0.0879], ["DP forward", "Z", 200, 50, 0.4285], ["NumPy tree", "Z", 200, 50, 7.431], ["PyTorch FFT", "Z", 200, 50, 4.0861], ["DP forward", "pi", 200, 50, 14.3637], ["NumPy tree", "pi", 200, 50, 20.9576], ["PyTorch FFT + autograd", "pi", 200, 50, 11.4638], ["NumPy tree (L-BFGS)", "fit", 200, 50, 2782.9883], ["R:sampling (pi)", "pi", 200, 50, 7.0], ["R:sampling (fit)", "fit", 200, 50, 26.0], ["R:sampling (1 sample, incl. DP)", "samples", 200, 50, 3.0], ["R:sampling (1 sample, excl. DP)", "samples", 200, 50, 0.1838], ["NumPy tree (1 sample)", "samples", 200, 50, 0.1968], ["PyTorch tree (1 sample)", "samples", 200, 50, 0.2479], ["Sequential (1 sample)", "samples", 200, 50, 0.1239], ["DP forward", "Z", 200, 100, 0.4373], ["NumPy tree", "Z", 200, 100, 7.8394], ["PyTorch FFT", "Z", 200, 100, 4.1363], ["DP forward", "pi", 200, 100, 27.9907], ["NumPy tree", "pi", 200, 100, 21.3661], ["PyTorch FFT + autograd", "pi", 200, 100, 11.2686], ["NumPy tree (L-BFGS)", "fit", 200, 100, 2013.2817], ["R:sampling (pi)", "pi", 200, 100, 13.0], ["R:sampling (fit)", "fit", 200, 100, 49.0], ["R:sampling (1 sample, incl. DP)", "samples", 200, 100, 5.0], ["R:sampling (1 sample, excl. DP)", "samples", 200, 100, 0.1916], ["NumPy tree (1 sample)", "samples", 200, 100, 0.2886], ["PyTorch tree (1 sample)", "samples", 200, 100, 0.311], ["Sequential (1 sample)", "samples", 200, 100, 0.102], ["DP forward", "Z", 500, 5, 0.968], ["NumPy tree", "Z", 500, 5, 15.9069], ["PyTorch FFT", "Z", 500, 5, 4.8652], ["DP forward", "pi", 500, 5, 4.6648], ["NumPy tree", "pi", 500, 5, 48.042], ["PyTorch FFT + autograd", "pi", 500, 5, 12.7491], ["NumPy tree (L-BFGS)", "fit", 500, 5, 9310.7465], ["R:sampling (pi)", "pi", 500, 5, 3.0], ["R:sampling (fit)", "fit", 500, 5, 11.0], ["R:sampling (1 sample, incl. DP)", "samples", 500, 5, 2.0], ["R:sampling (1 sample, excl. DP)", "samples", 500, 5, 0.3794], ["NumPy tree (1 sample)", "samples", 500, 5, 0.0388], ["PyTorch tree (1 sample)", "samples", 500, 5, 0.0472], ["Sequential (1 sample)", "samples", 500, 5, 0.1808], ["DP forward", "Z", 500, 10, 0.8246], ["NumPy tree", "Z", 500, 10, 14.1129], ["PyTorch FFT", "Z", 500, 10, 4.5231], ["DP forward", "pi", 500, 10, 7.4461], ["NumPy tree", "pi", 500, 10, 44.0294], ["PyTorch FFT + autograd", "pi", 500, 10, 11.7308], ["NumPy tree (L-BFGS)", "fit", 500, 10, 10909.6538], ["R:sampling (pi)", "pi", 500, 10, 5.0], ["R:sampling (fit)", "fit", 500, 10, 17.0], ["R:sampling (1 sample, incl. DP)", "samples", 500, 10, 3.0], ["R:sampling (1 sample, excl. DP)", "samples", 500, 10, 0.4], ["NumPy tree (1 sample)", "samples", 500, 10, 0.0668], ["PyTorch tree (1 sample)", "samples", 500, 10, 0.0751], ["Sequential (1 sample)", "samples", 500, 10, 0.2004], ["DP forward", "Z", 500, 20, 0.9933], ["NumPy tree", "Z", 500, 20, 16.1674], ["PyTorch FFT", "Z", 500, 20, 5.0006], ["DP forward", "pi", 500, 20, 15.7816], ["NumPy tree", "pi", 500, 20, 48.8788], ["PyTorch FFT + autograd", "pi", 500, 20, 12.9993], ["NumPy tree (L-BFGS)", "fit", 500, 20, 7710.9671], ["R:sampling (pi)", "pi", 500, 20, 9.0], ["R:sampling (fit)", "fit", 500, 20, 32.5], ["R:sampling (1 sample, incl. DP)", "samples", 500, 20, 5.0], ["R:sampling (1 sample, excl. DP)", "samples", 500, 20, 0.4545], ["NumPy tree (1 sample)", "samples", 500, 20, 0.1143], ["PyTorch tree (1 sample)", "samples", 500, 20, 0.1274], ["Sequential (1 sample)", "samples", 500, 20, 0.2023], ["DP forward", "Z", 500, 50, 0.9611], ["NumPy tree", "Z", 500, 50, 15.7036], ["PyTorch FFT", "Z", 500, 50, 5.1002], ["DP forward", "pi", 500, 50, 35.9888], ["NumPy tree", "pi", 500, 50, 47.565], ["PyTorch FFT + autograd", "pi", 500, 50, 13.7486], ["NumPy tree (L-BFGS)", "fit", 500, 50, 6120.3367], ["R:sampling (pi)", "pi", 500, 50, 18.5], ["R:sampling (fit)", "fit", 500, 50, 69.0], ["R:sampling (1 sample, incl. DP)", "samples", 500, 50, 9.0], ["R:sampling (1 sample, excl. DP)", "samples", 500, 50, 0.4432], ["NumPy tree (1 sample)", "samples", 500, 50, 0.237], ["PyTorch tree (1 sample)", "samples", 500, 50, 0.2965], ["Sequential (1 sample)", "samples", 500, 50, 0.2207], ["DP forward", "Z", 500, 100, 1.0872], ["NumPy tree", "Z", 500, 100, 15.9084], ["PyTorch FFT", "Z", 500, 100, 6.5057], ["DP forward", "pi", 500, 100, 72.3487], ["NumPy tree", "pi", 500, 100, 47.7589], ["PyTorch FFT + autograd", "pi", 500, 100, 12.5473], ["NumPy tree (L-BFGS)", "fit", 500, 100, 5750.0048], ["R:sampling (pi)", "pi", 500, 100, 32.0], ["R:sampling (fit)", "fit", 500, 100, 91.0], ["R:sampling (1 sample, incl. DP)", "samples", 500, 100, 14.0], ["R:sampling (1 sample, excl. DP)", "samples", 500, 100, 0.4236], ["NumPy tree (1 sample)", "samples", 500, 100, 0.4116], ["PyTorch tree (1 sample)", "samples", 500, 100, 0.4026], ["Sequential (1 sample)", "samples", 500, 100, 0.2211], ["DP forward", "Z", 500, 200, 1.2413], ["NumPy tree", "Z", 500, 200, 14.7462], ["PyTorch FFT", "Z", 500, 200, 4.77], ["DP forward", "pi", 500, 200, 129.2031], ["NumPy tree", "pi", 500, 200, 44.094], ["PyTorch FFT + autograd", "pi", 500, 200, 12.894], ["NumPy tree (L-BFGS)", "fit", 500, 200, 3809.7222], ["R:sampling (pi)", "pi", 500, 200, 64.5], ["R:sampling (fit)", "fit", 500, 200, 186.5], ["R:sampling (1 sample, incl. DP)", "samples", 500, 200, 26.0], ["R:sampling (1 sample, excl. DP)", "samples", 500, 200, 0.4545], ["NumPy tree (1 sample)", "samples", 500, 200, 0.6229], ["PyTorch tree (1 sample)", "samples", 500, 200, 0.6253], ["Sequential (1 sample)", "samples", 500, 200, 0.2267], ["DP forward", "Z", 500, 400, 1.5593], ["NumPy tree", "Z", 500, 400, 14.4686], ["PyTorch FFT", "Z", 500, 400, 4.7203], ["DP forward", "pi", 500, 400, 261.5121], ["NumPy tree", "pi", 500, 400, 44.2542], ["PyTorch FFT + autograd", "pi", 500, 400, 12.5885], ["NumPy tree (L-BFGS)", "fit", 500, 400, 1824.8383], ["R:sampling (pi)", "pi", 500, 400, 101.0], ["R:sampling (1 sample, incl. DP)", "samples", 500, 400, 36.0], ["R:sampling (1 sample, excl. DP)", "samples", 500, 400, 0.4334], ["NumPy tree (1 sample)", "samples", 500, 400, 0.9485], ["PyTorch tree (1 sample)", "samples", 500, 400, 0.9455], ["Sequential (1 sample)", "samples", 500, 400, 0.253], ["DP forward", "Z", 1000, 5, 1.7599], ["NumPy tree", "Z", 1000, 5, 29.4955], ["PyTorch FFT", "Z", 1000, 5, 8.0366], ["DP forward", "pi", 1000, 5, 8.8171], ["NumPy tree", "pi", 1000, 5, 104.0453], ["PyTorch FFT + autograd", "pi", 1000, 5, 20.6436], ["NumPy tree (L-BFGS)", "fit", 1000, 5, 1778.4147], ["R:sampling (pi)", "pi", 1000, 5, 7.0], ["R:sampling (fit)", "fit", 1000, 5, 19.0], ["R:sampling (1 sample, incl. DP)", "samples", 1000, 5, 5.0], ["R:sampling (1 sample, excl. DP)", "samples", 1000, 5, 1.0], ["NumPy tree (1 sample)", "samples", 1000, 5, 0.04], ["PyTorch tree (1 sample)", "samples", 1000, 5, 0.0501], ["Sequential (1 sample)", "samples", 1000, 5, 0.3333], ["DP forward", "Z", 1000, 10, 1.731], ["NumPy tree", "Z", 1000, 10, 28.4009], ["PyTorch FFT", "Z", 1000, 10, 7.9415], ["DP forward", "pi", 1000, 10, 14.8797], ["NumPy tree", "pi", 1000, 10, 98.2449], ["PyTorch FFT + autograd", "pi", 1000, 10, 20.4834], ["NumPy tree (L-BFGS)", "fit", 1000, 10, 1758.3823], ["R:sampling (pi)", "pi", 1000, 10, 10.0], ["R:sampling (fit)", "fit", 1000, 10, 29.0], ["R:sampling (1 sample, incl. DP)", "samples", 1000, 10, 7.0], ["R:sampling (1 sample, excl. DP)", "samples", 1000, 10, 0.8], ["NumPy tree (1 sample)", "samples", 1000, 10, 0.07], ["PyTorch tree (1 sample)", "samples", 1000, 10, 0.0858], ["Sequential (1 sample)", "samples", 1000, 10, 0.3809], ["DP forward", "Z", 1000, 20, 1.745], ["NumPy tree", "Z", 1000, 20, 28.3778], ["PyTorch FFT", "Z", 1000, 20, 7.905], ["DP forward", "pi", 1000, 20, 27.5195], ["NumPy tree", "pi", 1000, 20, 98.2721], ["PyTorch FFT + autograd", "pi", 1000, 20, 20.2101], ["NumPy tree (L-BFGS)", "fit", 1000, 20, 1846.7362], ["R:sampling (pi)", "pi", 1000, 20, 15.0], ["R:sampling (fit)", "fit", 1000, 20, 44.5], ["R:sampling (1 sample, incl. DP)", "samples", 1000, 20, 9.0], ["R:sampling (1 sample, excl. DP)", "samples", 1000, 20, 0.7], ["NumPy tree (1 sample)", "samples", 1000, 20, 0.1208], ["PyTorch tree (1 sample)", "samples", 1000, 20, 0.1487], ["Sequential (1 sample)", "samples", 1000, 20, 0.3899], ["DP forward", "Z", 1000, 50, 1.8611], ["NumPy tree", "Z", 1000, 50, 28.5913], ["PyTorch FFT", "Z", 1000, 50, 8.1125], ["DP forward", "pi", 1000, 50, 65.0406], ["NumPy tree", "pi", 1000, 50, 99.1488], ["PyTorch FFT + autograd", "pi", 1000, 50, 21.0208], ["NumPy tree (L-BFGS)", "fit", 1000, 50, 13915.2498], ["R:sampling (pi)", "pi", 1000, 50, 37.0], ["R:sampling (fit)", "fit", 1000, 50, 110.0], ["R:sampling (1 sample, incl. DP)", "samples", 1000, 50, 19.0], ["R:sampling (1 sample, excl. DP)", "samples", 1000, 50, 0.9], ["NumPy tree (1 sample)", "samples", 1000, 50, 0.3267], ["PyTorch tree (1 sample)", "samples", 1000, 50, 0.3453], ["Sequential (1 sample)", "samples", 1000, 50, 0.4811], ["DP forward", "Z", 1000, 100, 2.2453], ["NumPy tree", "Z", 1000, 100, 31.5853], ["PyTorch FFT", "Z", 1000, 100, 8.8324], ["DP forward", "pi", 1000, 100, 141.861], ["NumPy tree", "pi", 1000, 100, 106.8826], ["PyTorch FFT + autograd", "pi", 1000, 100, 22.4234], ["NumPy tree (L-BFGS)", "fit", 1000, 100, 11409.7555], ["R:sampling (pi)", "pi", 1000, 100, 65.0], ["R:sampling (fit)", "fit", 1000, 100, 191.0], ["R:sampling (1 sample, incl. DP)", "samples", 1000, 100, 31.0], ["R:sampling (1 sample, excl. DP)", "samples", 1000, 100, 1.0], ["NumPy tree (1 sample)", "samples", 1000, 100, 0.4927], ["PyTorch tree (1 sample)", "samples", 1000, 100, 0.4975], ["Sequential (1 sample)", "samples", 1000, 100, 0.4127], ["DP forward", "Z", 1000, 200, 2.3763], ["NumPy tree", "Z", 1000, 200, 28.7355], ["PyTorch FFT", "Z", 1000, 200, 8.2679], ["DP forward", "pi", 1000, 200, 258.7122], ["NumPy tree", "pi", 1000, 200, 99.7394], ["PyTorch FFT + autograd", "pi", 1000, 200, 21.0941], ["NumPy tree (L-BFGS)", "fit", 1000, 200, 11591.9656], ["R:sampling (pi)", "pi", 1000, 200, 115.0], ["R:sampling (fit)", "fit", 1000, 200, 342.0], ["R:sampling (1 sample, incl. DP)", "samples", 1000, 200, 52.0], ["R:sampling (1 sample, excl. DP)", "samples", 1000, 200, 0.8], ["NumPy tree (1 sample)", "samples", 1000, 200, 0.8863], ["PyTorch tree (1 sample)", "samples", 1000, 200, 0.8407], ["Sequential (1 sample)", "samples", 1000, 200, 0.473], ["DP forward", "Z", 1000, 400, 3.4032], ["NumPy tree", "Z", 1000, 400, 30.8499], ["PyTorch FFT", "Z", 1000, 400, 8.6692], ["DP forward", "pi", 1000, 400, 557.3375], ["NumPy tree", "pi", 1000, 400, 105.5471], ["PyTorch FFT + autograd", "pi", 1000, 400, 21.9332], ["NumPy tree (L-BFGS)", "fit", 1000, 400, 6682.717], ["R:sampling (pi)", "pi", 1000, 400, 253.0], ["R:sampling (fit)", "fit", 1000, 400, 716.5], ["R:sampling (1 sample, incl. DP)", "samples", 1000, 400, 97.0], ["R:sampling (1 sample, excl. DP)", "samples", 1000, 400, 1.0], ["NumPy tree (1 sample)", "samples", 1000, 400, 1.3233], ["PyTorch tree (1 sample)", "samples", 1000, 400, 1.2995], ["Sequential (1 sample)", "samples", 1000, 400, 0.4821]];
...
// Parse into {experiment: {method: {Ns, ns, ts}}}
var byExp = {};
raw.forEach(function(r) {
  var m = r[0], exp = r[1], N = r[2], n = r[3], t = r[4];
  if (!byExp[exp]) byExp[exp] = {};
  if (!byExp[exp][m]) byExp[exp][m] = {N:[], n:[], t:[]};
  byExp[exp][m].N.push(N);
  byExp[exp][m].n.push(n);
  byExp[exp][m].t.push(t);
});

function makeTraces(exp) {
  var methods = byExp[exp];
  var traces = [];
  Object.keys(methods).forEach(function(m) {
    var d = methods[m];
    var color = COLORS[m] || '#888';
    var cplx = COMPLEXITY[m] || '';
    traces.push({
      type: 'scatter3d', mode: 'markers',
      x: d.N.map(Math.log10), y: d.n.map(Math.log10),
      z: d.t.map(Math.log10),
      marker: {size: 3, color: color, opacity: 0.9},
      name: m + '  ' + cplx,
      hovertemplate: m + '<br>N=%{customdata[0]}, n=%{customdata[1]}<br>%{customdata[2]:.2f} ms<extra></extra>',
      customdata: d.N.map(function(_,i){return [d.N[i], d.n[i], d.t[i]]})
    });
    // Add surface: build z-matrix on the (N, n) grid
    var uN = Array.from(new Set(d.N)).sort(function(a,b){return a-b});
    var un = Array.from(new Set(d.n)).sort(function(a,b){return a-b});
    var lookup = {};
    d.N.forEach(function(_,i){ lookup[d.N[i]+','+d.n[i]] = d.t[i]; });
    // z_matrix[i][j] = log10(time) for uN[i], un[j]; null where missing
    var z_matrix = uN.map(function(N) {
      return un.map(function(n) {
        var v = lookup[N+','+n];
        return v !== undefined ? Math.log10(v) : null;
      });
    });
    if (uN.length >= 2 && un.length >= 2) {
      // surface: z[i][j] maps to (x[j], y[i]), so rows=N, cols=n
      // but scatter3d has x=N, y=n — transpose so x=N (cols), y=n (rows)
      var z_T = un.map(function(_,j) {
        return uN.map(function(_,i) { return z_matrix[i][j]; });
      });
      traces.push({
        type: 'surface',
        x: uN.map(Math.log10),   // N axis (columns of z_T)
        y: un.map(Math.log10),   // n axis (rows of z_T)
        z: z_T,
        colorscale: [[0, color], [1, color]],
        opacity: 0.35, showscale: false,
        name: m + ' (surface)', showlegend: false,
...
var layout = {
  ...
  updatemenus: [{
    ...
    buttons: [
      {label: 'Computing Z', method: 'restyle',
       args: [makeTraces('Z').reduce(function(a,t,i){
         Object.keys(t).forEach(function(k){ if(!a[k]) a[k]=[]; a[k].push(t[k]); });
         return a;
       }, {})]},
      {label: 'Computing \u03c0', method: 'restyle',
       args: [makeTraces('pi').reduce(function(a,t,i){
         Object.keys(t).forEach(function(k){ if(!a[k]) a[k]=[]; a[k].push(t[k]); });
         return a;
       }, {})]},
    ]
...
// Start with pi (the main event)
Plotly.newPlot('timing-3d', makeTraces('pi'), layout, {responsive: true});

// Fix dropdown: simpler approach — just replace data on click
...
select.addEventListener('change', function() {
  Plotly.react('timing-3d', makeTraces(this.value), layout);
});

// Remove the plotly dropdown since we have our own
layout.updatemenus = [];
Plotly.react('timing-3d', makeTraces('pi'), layout);
})();
</script>
