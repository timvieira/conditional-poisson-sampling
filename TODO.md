...
- [ ] Add a diagram explaining what the "contour" is a contour *of* — the coefficient extraction integral $\llbracket f \rrbracket(z^n) = \frac{1}{2\pi i}\oint f(z)\,z^{-n-1}\,dz$ is over a circle in the complex plane. The radius $r$ of that circle is the free parameter we're optimizing. A diagram showing the complex plane with the generating function's poles/zeros and the integration contour at different radii would make this concrete. Currently the section assumes the reader knows Cauchy's integral formula.
- [ ] Add numerical validation of the contour radius: verify that the chosen $r$ actually minimizes (or nearly minimizes) the relative error in the extracted coefficient. Compare $\llbracket f \rrbracket(z^n)$ computed at the optimal $r$ vs. at $r=1$ (no scaling) and at a few other values. This would make the "optimal" claim empirically testable and show readers *why* scaling matters.
- [ ] Fix complexity column in summary table: replace "$+O(1)\times$" with actual bounds. Every row should state its own complexity clearly: Z is O(N log² n), π is O(N log² n), Cov·v is O(N log² n), samples are O(n log N) each. The Baur-Strassen constant (≤5× arithmetic ops) and Pearlmutter bound can be mentioned in the body text with proper citations, but the table should just give the complexity of each operation. Same fix needed in the fitting section's transformation table.
- [ ] Fitting cost needs to be stated precisely: total cost = (number of iterations) × (cost per iteration). Cost per iteration = L-BFGS two-loop recursion O(mN) + line search × (function eval O(N log² n) + gradient eval O(N log² n)). The line search may require multiple function+gradient evaluations per iteration (Liu & Nocedal use cubic interpolation). State the per-iteration cost breakdown, don't just wave at "L-BFGS".
...
- [ ] Explain or reference the FFT trick for polynomial multiplication — the blog assumes readers know that convolving two coefficient arrays can be done in O(d log d) via FFT (pointwise multiply in frequency domain, then IFFT back). A one-paragraph explanation or a pointer to a good reference would help readers who know polynomials but not the FFT connection. This is the single fact that turns O(d²) convolution into O(d log d) and makes the whole tree O(N log² n) instead of O(Nn). Good references: 3Blue1Brown's ["But what is a convolution?"](https://www.3blue1brown.com/lessons/convolutions) (has a section on polynomial multiplication + FFT speedup, minimal Fourier prerequisites), CLRS Ch. 30.
